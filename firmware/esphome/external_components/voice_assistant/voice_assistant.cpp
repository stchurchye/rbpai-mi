...
void VoiceAssistant::signal_audio_end_() {
  if (this->api_client_ == nullptr) {
    return;
  }
  ESP_LOGD(TAG, "Signaling audio end");

  if (this->ring_buffer_ != nullptr) {
    bool is_available = this->ring_buffer_->available() >= SEND_BUFFER_SIZE;
    bool is_available2 = false;
    if (this->mic_source2_ && this->ring_buffer2_ != nullptr) {
      is_available2 = this->ring_buffer2_->available() >= SEND_BUFFER_SIZE;
    }
    ...
    size_t remaining = this->ring_buffer_->available();
    size_t remaining2 = 0;
    if (this->mic_source2_ && this->ring_buffer2_ != nullptr) {
      remaining2 = this->ring_buffer2_->available();
    }
    ...
    if (remaining > 0 || remaining2 > 0) {
      ...
      if (remaining > 0) {
        size_t read_bytes = this->ring_buffer_->read((void *) this->send_buffer_, remaining, 0);
        msg.data = this->send_buffer_;
        msg.data_len = read_bytes;
      }

      if (remaining2 > 0) {
        size_t read_bytes = this->ring_buffer2_->read((void *) this->send_buffer2_, remaining2, 0);
        msg.data2 = this->send_buffer2_;
        msg.data2_len = read_bytes;
      }
  ...
  api::VoiceAssistantAudio end_msg;
  end_msg.end = true;
  this->api_client_->send_message(end_msg);
}
